...
test('20 - Re-Initialize button', async ({ page }) => {
  ...
  // 25 - Switch data distribution
    test('25 - Switch data distribution to ', async ({ page }) => {
    await page.goto('http://localhost:3000/');
    await page.getByRole('combobox').first().selectOption('Scattered'); // Assuming option exists
    await page.locator('[id="slider::r2::track"]').click();
    await page.getByRole('combobox').nth(3).selectOption('Random');
    await page.getByRole('button', { name: 'Re-Initialize' }).click();
    await expect(page.getByText(/Initialization Finished/i)).toBeVisible();
...
// 26 - Check Start and Stop buttons
...
// 27 - Check if adding points works
test('27 - Check if adding points works', async ({ page }) => {
  await page.goto('http://localhost:3000/');
  await page.getByRole('combobox').first().selectOption('grid');
...
// 28 - Check if removing points works
test('28 - Check if removing points works', async ({ page }) => {
  await page.goto('http://localhost:3000/');
  await page.getByRole('combobox').first().selectOption('grid');
  await page.getByRole('button', { name: 'Points' }).click();
  await page.locator('canvas').click({
...
// 29 - Check if adding centroids works
test('29 - Check if adding centroids works', async ({ page }) => {
  await page.goto('http://localhost:3000/');
  await page.getByRole('combobox').first().selectOption('grid');
  await page.getByRole('slider').first().click();
  await page.getByText('1').first().click();
...
// 30 - Check if removing centroids works
test('30 - Check if removing centroids works', async ({ page }) => {
  await page.goto('http://localhost:3000/');
  await page.getByRole('combobox').first().selectOption('grid');
    await page.getByRole('slider').first().click();
    await page.keyboard.press('End'); 
